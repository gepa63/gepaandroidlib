...
public class SimpleFileDialog 
{
	...
	public SimpleFileDialog(Context context, String file_select_type, SimpleFileDialogListener simpleFileDialogListener, boolean useExternalStorage)
	{
		this( context, file_select_type, simpleFileDialogListener, SimpleFileDialog.eval(context, useExternalStorage) );
	}	
	private static String eval(Context context, boolean useExternalStorage) {
		
		String absPath = "";
		String appName = "QuickPhoneCalling";
		try{
			absPath = Environment.getExternalStorageDirectory().getAbsolutePath();
		} catch(Exception ex){}
		try{
			appName = SystemInfo.getApplicationName( context );
		} catch(Exception ex){}
		return useExternalStorage ?  absPath : appName;
	}
	public SimpleFileDialog(Context context, String file_select_type, SimpleFileDialogListener SimpleFileDialogListener, String startDirectory)
	{
		...
		m_context = context;
		m_sdcardDirectory = startDirectory;
		m_SimpleFileDialogListener = SimpleFileDialogListener;

		try
		{
			File f = new File(m_sdcardDirectory);
			m_sdcardDirectory = f.getCanonicalPath();
			if( !f.exists() )
				f.mkdirs();
		}
	...
	public void setDefault_File_Name(String def) 
	{
		if( def.contains("/" ) )
		{
			File f = new File(def);
			this.Default_File_Name = f.getName();
			String path = f.getAbsolutePath();
			this.m_sdcardDirectory = path.substring(0, path.lastIndexOf("/") );
		}
		else
			this.Default_File_Name = def;
	}
} 
